...
async function run(options: RunOptions = {}) {
  ...
  const isForegroundMode = process.env.FOREGROUND_MODE === "true";

  // Only save PID file in background mode
  if (!isForegroundMode) {
    savePid(process.pid);
  }
  ...
  const loggerConfig =
    config.LOG !== false
      ? {
          ...
          // Foreground mode outputs to terminal, background mode outputs to file
          stream: isForegroundMode
            ? process.stdout
    ...
    if (req.sessionId && req.url.startsWith("/v1/messages") && !req.url.startsWith("/v1/messages/count_tokens")) {
      if (payload instanceof ReadableStream) {
        if (req.agents) {
          ...
          // Store Anthropic-format message body, distinguishing between text and tool types
