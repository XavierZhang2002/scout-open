...
import { process_messages, handle_system_message } from "./hook";
    ...
    try {
      ...
      this.app.addHook("preHandler", (request, reply, done) => {
        if (request.url.startsWith("/v1/messages") && request.body) {
          // Expand data content to top level, keep other info at the same level
          let first_user_message = "";
          if (request.body.messages && Array.isArray(request.body.messages)) {
            first_user_message = request.body.messages[0].content[0].text;
          }
          ...
          // Apply hook to process messages array
          if (request.body.messages && Array.isArray(request.body.messages)) {
            try {
              const processedMessages = process_messages(request.body.messages, {
                request,
                reply,
                log: request.log
              });
              ...
              request.log.error("Error processing messages with hook:", error);
            }
          }

          // Apply hook to process system field
          if (request.body.system && Array.isArray(request.body.system)) {
            try {
              const processedSystem = request.body.system.map((item: any) => {
                if (item.type === 'text' && typeof item.text === 'string') {
                  const processedText = handle_system_message(item.text, {
                    request,
                    reply,
                    log: request.log
                  });
              ...
              request.body.system = processedSystem;
            } catch (error) {
              request.log.error("Error processing system field with hook:", error);
            }
          }
        }
        let first_user_message = "";
        if (request && request.body && request.body.messages && Array.isArray(request.body.messages)) {
          first_user_message = request.body.messages[0].content[0].text;
        }
        const logData = { ...request.body, first_user_message, type: "request body" };
